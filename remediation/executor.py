...
still doesn't, directly — Cavendex never runs a local shell command,
...
Cavendex asks; whatever's on the other end of that webhook decides
...
- CAVENDEX_REMEDIATION_ENABLED (default false) — the master switch.
- CAVENDEX_REMEDIATION_ACTION_TYPES (default "block_ip,isolate_host")
...
CAVENDEX_REMEDIATION_DRY_RUN (default true whenever remediation is
...
def _remediation_enabled() -> bool:
    return os.getenv("CAVENDEX_REMEDIATION_ENABLED", "false").strip().lower() in ("1", "true", "yes")


def _dry_run() -> bool:
    return os.getenv("CAVENDEX_REMEDIATION_DRY_RUN", "true").strip().lower() in ("1", "true", "yes")


def _webhook_url() -> str:
    return os.getenv("CAVENDEX_REMEDIATION_WEBHOOK_URL", "").strip()


def _signing_secret() -> str:
    return os.getenv("CAVENDEX_REMEDIATION_WEBHOOK_SIGNING_SECRET", "").strip()


def _allowed_action_types() -> set:
    raw = os.getenv("CAVENDEX_REMEDIATION_ACTION_TYPES", "block_ip,isolate_host")
...
def is_automatable(action_type: str) -> bool:
    ...
    CAVENDEX_REMEDIATION_ACTION_TYPES — it's the Responder's fallback
...
def sign_payload(body: bytes, secret: str) -> str:
    ...
    (CAVENDEX_REMEDIATION_WEBHOOK_SIGNING_SECRET) since a remediation
...
def send_remediation_request(payload: dict) -> dict:
    """POST `payload` to CAVENDEX_REMEDIATION_WEBHOOK_URL, or simulate
    doing so under CAVENDEX_REMEDIATION_DRY_RUN (checked first — dry
    ...
    """
    if _dry_run():
        return {"outcome": "dry_run", "detail": "CAVENDEX_REMEDIATION_DRY_RUN is set; nothing was sent"}

    url = _webhook_url()
    if not url:
        return {"outcome": "not_configured", "detail": "CAVENDEX_REMEDIATION_WEBHOOK_URL is not set"}
