...
- **An analyst-facing web dashboard, not just a CLI/API.** Open `http://localhost:8000/` for a single-page, no-build-step HTML/CSS/JS UI (served from `static/`, works fully offline — no CDN dependencies) with an incident queue, live agent findings, threat-intel/ATT&CK sections, one-click approve/deny with an attributed analyst name, a live-streaming "new incident" form, and a standalone threat-hunt form.
...
2. **Fuzzy** — two narrower signals, each grounded in a specific attacker behavior: a **subnet** match (two IPs in the same `/24`, IPv4, or `/64`, IPv6 — rotating source IPs is a standard evasion of exact-IP matching), and a **domain family** match (two domains sharing the same registered/apex domain, resolved via the real Public Suffix List through `tldextract`, not a naive last-two-labels split — since C2 infrastructure reusing one domain across subdomains is standard). Both tiers are free — pure Python, no LLM call.
...
Every item on the originally-stated priority list (ingestion → threat-intel enrichment → dashboard → alert correlation) and all of Phase 8 (semantic correlation, Public Suffix List domain matching, Shodan enrichment, syslog listener + polling connector) has shipped. This phase closed four more real gaps: ingestion-layer rate limiting, named-analyst approval accountability, a permanent prompt-injection regression test, webhook alerting, and off-box vault backup — plus a real, purpose-built Wazuh integration (normalizer + tail/push paths), the first vendor-specific SIEM/EDR connector this project has shipped rather than only the generic polling/ingestion mechanism.

**Next candidates**
- Tamper-evidence for the audit trail (hash chain or append-only store) — the newly identified gap above
...
- Per-tenant user/role accounts, and server-side sessions for the dashboard
