...
- **Built for a team working the same queue, not just one analyst.** Inline analyst notes and incident ownership/assignment (both free-text accountability labels, like `approved_by` — see Security Notes) let more than one person coordinate on the same incident. A dedicated "ATT&CK Overview" tab aggregates every verified technique cited across a tenant's incidents, grouped by tactic in kill-chain order. The incident list updates itself in real time — an in-process pub/sub publishes on every pipeline write, pushed to open dashboard tabs over SSE with reconnect-with-backoff — so an incident another analyst just approved, or one a running pipeline just created, appears without waiting for the next poll; 15-second polling stays on as a fallback if the push connection is ever unavailable (a real risk behind some reverse proxies — see DEPLOYMENT.md).
...
3. **Identity** (`ingestion/identity_correlation.py`) — free and deterministic like the fuzzy tier, but catches what neither exact nor fuzzy structurally can: a *renamed* asset, or a lexically-unrelated second-stage domain. Two independent signals, each opt-in by simply being unconfigured: an **asset identity** lookup against an operator-supplied inventory/CMDB export (`utils/asset_inventory.py` — see `SENTINELOS_ASSET_INVENTORY_PATH` in `.env.example`), and an opt-in **domain identity** lookup via real passive DNS (`SENTINELOS_CORRELATION_IDENTITY_DNS_ENABLED`, off by default since it spends a real VirusTotal API call per domain IOC) — two lexically-unrelated domains that have both resolved to the same IP are plausibly the same C2 infrastructure at a different stage.
...
- **The exact/fuzzy/identity tiers can rescue an alert the severity filter would otherwise drop; the semantic tier deliberately cannot.** Exact/fuzzy/identity are all checked before `SENTINELOS_INGEST_MIN_SEVERITY` (they're free); semantic correlation is checked after it, only for an alert already about to trigger a full pipeline run — so a semantic match *replaces* that cost rather than adding a new one to routine noise.
...
`remediation/` closes the gap DEPLOYMENT.md's Known Limitations used to state plainly: "the Responder Agent never executes anything against a real system." It still doesn't, directly — SentinelOS never runs a local shell command, touches a firewall, or calls one specific EDR/SOAR vendor's API. Instead, an already-approved action whose type is eligible gets POSTed to your own webhook, the same "no vendor hardcoded, generic outbound POST" principle `notifications/webhook.py` already uses — your own SOAR platform, firewall API gateway, or custom script decides how (and whether) to actually act, sandboxed by construction on infrastructure you control and are responsible for validating.
...
- **Remediation execution is opt-in and sandboxed by construction, never a local shell command or a hardcoded vendor API call.** By default the Responder Agent still only ever proposes actions as data (action/target/rationale) and nothing executes them. `SENTINELOS_REMEDIATION_ENABLED=true` lets an *already-approved* action whose Responder-assigned `action_type` is eligible (`block_ip`/`isolate_host` by default; `action_type: "other"` — the Responder's fallback when no cleaner category applies — is never eligible regardless of config) get POSTed to your own webhook, the same "no vendor hardcoded" principle as the alerting webhook: SentinelOS asks, your own SOAR platform/firewall gateway/script decides how (and whether) to actually act. `SENTINELOS_REMEDIATION_DRY_RUN=true` (the default whenever remediation is enabled at all) is the literal sandbox — nothing is sent over the network, every eligible action is logged exactly as it would have been sent, so the wiring can be verified before it's live. See "Remediation" below.
...
- **Real remediation execution exists now, but only reaches as far as your own webhook.** `remediation/` (see "Remediation" above) can POST an approved, eligible action to an operator-configured webhook — off by default, and dry-run by default even when enabled — but SentinelOS itself still never calls a firewall/EDR/IAM API directly; whatever real action actually happens is entirely up to what your webhook receiver does with the request, which this project has no way to verify from here. Only `block_ip`/`isolate_host` are eligible out of the box (`SENTINELOS_REMEDIATION_ACTION_TYPES`); `disable_account`/`reset_credentials` exist as action_type categories but aren't in the default allowlist, since account/credential actions typically need more receiver-side context (which account system, what "reset" even means there) than a firewall block does.
- The dashboard has no server-side sessions or per-user accounts — its API-key auth is a `localStorage`-held key sent on every request, correct for a single-analyst or small trusted team, not enterprise multi-user auth. Assignment/notes attribution follows the same honest-label pattern as `approved_by`: freely typed, not tied to a login.
- **Live push updates are in-process only, not multi-worker.** The pub/sub behind the dashboard's real-time list updates (`utils/incident_events.py`) is a plain in-memory `queue.Queue` registry, matching every other piece of process-local state in this project (rate limiting, dedup, correlation — see below) — it does not fan out across multiple `uvicorn` worker processes or replicas behind a load balancer. A dashboard tab connected to a different worker than the one that persisted a change won't see that push (though its 15-second polling fallback still catches it). Fine for the single-process deployment this project targets; a real multi-worker deployment needs a shared broker (Redis pub/sub or similar) instead.
- **The Strand Map's labels crowd at high indicator-reuse density.** Every incident label is always drawn, unconditionally — fine at realistic tenant sizes, but a synthetic stress test (60 incidents sharing only 6 IOCs) produced visibly overlapping text where many incidents converge on the same few shared nodes. The underlying graph data and physics layout are both correct at that density (live-verified — see Testing); this is a rendering/legibility limit, not a data-correctness one. A future pass could hide incident labels until hover/zoom-in past a node-count threshold, the same tradeoff most link-analysis tools make, rather than always-on labeling.
...
- Hiding the Strand Map's incident labels until hover/zoom past a node-count threshold — a real, honestly-documented legibility limit at high indicator-reuse density (see Known Gaps), not a data-correctness bug
